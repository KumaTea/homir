...
func (m *Manager) Serve(w http.ResponseWriter, r *http.Request, upstreamName, artifactPath string) {
	...
	key := cacheKey(upstreamName, artifactPath)
	...
	s := m.getOrStart(key, upstreamName, artifactPath, upstream)
	m.serveSession(w, r, s)
}

func (m *Manager) getOrStart(key, upstreamName, artifactPath string, upstream config.Upstream) *session {
	m.mu.Lock()
	defer m.mu.Unlock()
	if s := m.sessions[key]; s != nil {
		return s
	}
	s := &session{key: key, partPath: filepath.Join(m.partial, key+".part"), total: -1, ready: make(chan struct{}), done: make(chan struct{})}
...
func (m *Manager) download(s *session, upstreamName, artifactPath string, upstream config.Upstream) {
	...
	response, err := m.fetch(upstream, artifactPath)
	if err != nil {
		s.finish(err)
	...
	if err := os.Rename(s.partPath, filepath.Join(m.artifacts, filename)); err != nil {
		s.finish(err)
		return
	}
	s.mu.Lock()
	...
	s.mu.Unlock()
	if err := m.store.Complete(store.Artifact{Key: s.key, Upstream: upstreamName, Path: artifactPath, Filename: filename, Size: size, ContentType: contentType}); err != nil {
		s.finish(err)
		return
	}
	s.finish(nil)
...
func (m *Manager) fetch(upstream config.Upstream, artifactPath string) (*http.Response, error) {
...
func (m *Manager) serveSession(w http.ResponseWriter, r *http.Request, s *session) {
	...
	s.mu.Lock()
	err, total, contentType := s.err, s.total, s.contentType
	s.mu.Unlock()
	if err != nil {
...
func cacheKey(upstream, artifactPath string) string {
